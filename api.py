...
async def get_letter(data:dict):
    async with aiohttp.ClientSession() as session:
        async with session.post(f'{API_HOST}/cover_letters/', json=data) as response:
            return await response.json()


async def get_skills(data: dict):
    async with aiohttp.ClientSession() as session:
        async with session.post(f'{API_HOST}/recognize_req/', json=data) as response:
            return await response.json()
...
if __name__ == '__main__':
    ...
    print(asyncio.run(get_letter({'company': 'Шарага', 'position': 'разраб', 'interest': 'рокетам', 'requirements': ["Python","Kafka",
    "Docker",
    "Django Framework",
    "MongoDB",
    "RabbitMQ",
